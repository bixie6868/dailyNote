...
* ROME 定位部分Locate
  * ---正在看---
  * 怎么选取的某一层？如何进行定位比如llama可以定位到第5层进行编辑的？
    * 看文章发现gpt-2是定位到第17层，为什么MLP相较于其他层影响更大呢？
    * 在处理主题词的最后一个令牌时，中间层的MLP模块具有显著的因果效应，这表明它们在事实预测中起到了决定性作用。
    * 在图3中，通过修改计算图来隔离MLP模块的影响，发现当MLP模块的活动被切断时，底层的状态失去了它们的因果效应，这表明MLP模块对于维持事实回忆至关重要，attention更多的是用于生成next token。
    * mlp是当主题词输入时其中就已经存储了大部分的关于主题词的信息，进而在attention层中从大量的信息聚合检索出正确的答案。所以mlp层对于答案的预测至关重要
### 计划：如何定位，然后编辑。能不能将该方法应用到多模态大模型上。
* next token next token生成的话，换成多项选择就不对了
* 不要编辑安全的神经元，只编辑其他神经元。
